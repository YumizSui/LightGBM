...
/*!
 * \brief Objective function for LambdaLossNDCG2pp with NDCG
 */
class LambdaLossNDCG2pp : public RankingObjective {
 public:
  explicit LambdaLossNDCG2pp(const Config& config)
      : RankingObjective(config),
        sigmoid_(config.sigmoid),
        norm_(config.lambdarank_norm),
        truncation_level_(config.lambdarank_truncation_level),
        rho_coef_(config.lambdaloss2pp_rho),
        delta_coef_(config.lambdaloss2pp_delta) {
    label_gain_ = config.label_gain;
    // initialize DCG calculator
    DCGCalculator::DefaultLabelGain(&label_gain_);
    DCGCalculator::Init(label_gain_);
  ...
  explicit LambdaLossNDCG2pp(const std::vector<std::string>& strs)
      : RankingObjective(strs) {}
...
#pragma omp parallel for schedule(static)
  ...
  inline void GetGradientsForOneQuery(data_size_t query_id, data_size_t cnt,
    ...
    for (data_size_t i = 0; i < cnt - 1 && i < truncation_level_; ++i) {
      ...
      for (data_size_t j = i + 1; j < cnt; ++j) {
        ...
        // get dcg gap
        const double dcg_gap = high_label_gain - low_label_gain;
        // get discount of rho this pair
        const double high_discount = DCGCalculator::GetDiscount(high_rank);
        const double low_discount = DCGCalculator::GetDiscount(low_rank);
        const double rho_discount = fabs(high_discount - low_discount);
        // get discount of delta of this pair
        const int delta_rank = fabs(high_rank-low_rank);
        const double delta_discount_1 = DCGCalculator::GetDiscount(delta_rank-1);
        const double delta_discount_2 = DCGCalculator::GetDiscount(delta_rank);
        const double delta_discount = fabs(delta_discount_1 - delta_discount_2);
        // get discount of delta of pair
        const double paired_discount = rho_coef_ * rho_discount + delta_coef_ * delta_discount;
        // get delta NDCG
        double delta_pair_NDCG = dcg_gap * paired_discount * inverse_max_dcg;
        // regular the delta_pair_NDCG by score distance
        if (norm_ && best_score != worst_score) {
  ...
  const char* GetName() const override { return "lambdaloss2pp"; }
 ...
 private:
  ...
  /*! \brief Truncation position for max DCG */
  int truncation_level_;
  /*! \brief Constant of rho_ij (lambdarank) */
  double rho_coef_;
  /*! \brief Constant of delta_ij (lambdaloss2)) */
  double delta_coef_;
  /*! \brief Cache inverse max DCG, speed up calculation */
  std::vector<double> inverse_max_dcgs_;
  /*! \brief Cache result for sigmoid transform to speed up */
...
/*!
 * \brief Objective function for RankADCG
 */
class RankADCG : public RankingObjective {
 public:
  explicit RankADCG(const Config& config)
      : RankingObjective(config),
        sigmoid_(config.sigmoid),
        norm_(config.lambdarank_norm),
        truncation_level_(config.lambdarank_truncation_level),
  ...
  explicit RankADCG(const std::vector<std::string>& strs)
      : RankingObjective(strs) {}
  ...
  inline void GetGradientsForOneQuery(data_size_t query_id, data_size_t cnt,
    ...
    for (data_size_t i = 0; i < cnt - 1 && i < truncation_level_; ++i) {
      ...
      for (data_size_t j = i + 1; j < cnt; ++j) {
        ...
        // get discount of gamma of this pair(ADCG)
        const double gamma_discount = fabs((delta_rank)*delta_discount_1 - (delta_rank+1)*delta_discount_2);
        // get discount of delta of pair
        const double paired_discount = rho_coef_ * rho_discount + delta_coef_ * (delta_discount + gamma_discount);
        // get delta NDCG
        double delta_pair_NDCG = dcg_gap * paired_discount * inverse_max_dcg;
        // regular the delta_pair_NDCG by score distance
        if (norm_ && best_score != worst_score) {
...
/*!
 * \brief Objective function for LambdaLoss with NEDCG
 */
class RankNEDCG : public RankingObjective {
 public:
  explicit RankNEDCG(const Config& config)
      : RankingObjective(config),
        sigmoid_(config.sigmoid),
        norm_(config.lambdarank_norm),
        truncation_level_(config.lambdarank_truncation_level),
  ...
  explicit RankNEDCG(const std::vector<std::string>& strs)
      : RankingObjective(strs) {}
  ...
  inline void GetGradientsForOneQuery(data_size_t query_id, data_size_t cnt,
    ...
    for (data_size_t i = 0; i < cnt - 1 && i < truncation_level_; ++i) {
      ...
      for (data_size_t j = i + 1; j < cnt; ++j) {
        ...
        const double paired_discount = rho_coef_ * rho_discount + delta_coef_ * delta_discount;
        // get delta NEDCG
        double delta_pair_NEDCG = dcg_gap * paired_discount * inverse_max_dcg;
        // regular the delta_pair_NEDCG by score distance
        if (norm_ && best_score != worst_score) {
          delta_pair_NEDCG /= (0.01f + fabs(delta_score));
        }
        // calculate lambda for this pair
        double p_lambda = GetSigmoid(delta_score);
        double p_hessian = p_lambda * (1.0f - p_lambda);
        // update
        p_lambda *= -sigmoid_ * delta_pair_NEDCG;
        p_hessian *= sigmoid_ * sigmoid_ * delta_pair_NEDCG;
        lambdas[low] -= static_cast<score_t>(p_lambda);
        hessians[low] += static_cast<score_t>(p_hessian);
        lambdas[high] += static_cast<score_t>(p_lambda);
        hessians[high] += static_cast<score_t>(p_hessian);
